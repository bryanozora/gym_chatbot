...
def triage_agent_instructions(context_variables):
    ...
    1. If the user asks a question related to the document or exercises, use the handoff function to retrieve data from the RAG agent.
    2. If there is a number of sets/reps in your answer, modify it to {modifier} times of the original.
    3. If the user asks for a training regimen or anything of sorts, use the handoff function to retrieve data and modify the amount of training to {modifier} times of the original. You DO NOT need to change or add anything—simply adjust the amount to {modifier} times of the original.
    4. If the user asks about BMI (body mass index) or is mentioning anything related to someone's (user or someone else) weight and/or height, handoff to the BMI agent which will calculate the BMI of said person.
    5. If the user wants to register a new member, use the register_member function to pass the registration details.
    6. If the query does not relate to the document, exercises, BMI, or member registration, respond that you do not know.
...
# Registration Agent instructions
def registration_agent_instructions(context_variables):
    return """Register a member with full_name, height(cm), and weight(kg) and you MUST use register_member to store the data to the database."""
...
def handoff_to_registration_agent(query):
    print("HANDOFF TO REGIST AGENT")
...
def register_member(full_name, weight: int, height: int, csv_filepath="members.csv"):
    """
    Register & store member information to a database. weight in kg, height in cm.
    """

    print("========== Called Register Member ============")
    print(f"Full = {full_name}, Weight = {weight}, Height = {height}")
    
    try:
        weight = float(weight)
        height = float(height)
    except ValueError:
        return "Invalid weight or height provided."
    ...
    # Check if file exists to write headers or append data
    file_exists = os.path.isfile(csv_filepath)
    with open(csv_filepath, mode='a', newline='') as file:
        writer = csv.DictWriter(file, fieldnames=member_data.keys())
        if not file_exists:
            writer.writeheader()
        writer.writerow(member_data)

    print("REGISTERED!")
    return f"Member {full_name} registered successfully with BMI {round(bmi, 2)} ({classification})."

# Define the agents
triage_agent = Agent(
    name="Triage Agent",
    instructions=triage_agent_instructions,
    functions=[handoff, handoff_to_bmi_agent, handoff_to_registration_agent],
...
swarm_client.agents = [triage_agent, rag_agent, pdf_agent, bmi_agent, registration_agent]
